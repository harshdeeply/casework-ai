...
export type Case = Ticket & { id: string; finding: Finding; evidenceIds: string[]; evidenceSignature: string; evidenceSnapshot: Attempt[]; draft: string; finalMessage?: string; status: 'pending_review' | 'approved' | 'rejected' | 'handed_off'; reviewer?: string; createdAt: string; audit: Audit[] }
export type State = { version: 1; attempts: Attempt[]; cases: Case[]; desk: { handoffId: string; ticketId: string; message: string; at: string }[] }
...
export const draftFor = (ticket: Ticket, finding: Finding, attempts: Attempt[]): string => {
  ...
  switch (finding) {
    ...
    case 'RETRY_UNOBSERVED': return `A retry for event ${ticket.event_id} was scheduled for ${latest?.next_retry_at}, but no later attempt appears in the available logs. We are investigating the gap rather than assuming delivery succeeded or failed.`
...
export const isCurrent = (state: State, c: Case) => signature(scopedAttempts(state, c)) === c.evidenceSignature
...
export function openCase(state: State, ticket: Ticket): State {
  ...
  const attempts = scopedAttempts(state, ticket), finding = findingFor(attempts)
  return { ...state, cases: [{ ...ticket, id: id(), finding, evidenceIds: evidenceIds(finding, attempts), evidenceSignature: signature(attempts), evidenceSnapshot: attempts, draft: draftFor(ticket, finding, attempts), status: 'pending_review', createdAt: now(), audit: [{ action: 'opened', actor: 'system', at: now(), detail: finding }] }, ...state.cases] }
}
export function refreshCase(state: State, caseId: string): State {
  ...
  const attempts = scopedAttempts(state, c), finding = findingFor(attempts)
  return { ...state, cases: state.cases.map(x => x.id === caseId ? { ...x, finding, evidenceIds: evidenceIds(finding, attempts), evidenceSignature: signature(attempts), evidenceSnapshot: attempts, draft: draftFor(c, finding, attempts), audit: [...x.audit, { action: 'refreshed', actor: 'system', at: now(), detail: finding }] } : x) }
}
...
export function seedState(): State {
  ...
  let state: State = { version: 1, attempts: incident.attempts as Attempt[], cases: [], desk: [] }
