...
For a sanitized delivery export, run `python3 -m casework.importer examples/attempts.jsonl --dry-run`, then omit `--dry-run` to atomically load it into a chosen `--db` file. CSV is also supported (`examples/attempts.csv`). Conflicting records roll back the whole batch.
...
3. **Ground:** Snapshot the exact attempts and one versioned runbook entry. The code decides the finding; a model cannot change it. Drafts must cite available attempt and runbook IDs.
4. **Review:** A reviewer can edit, approve or reject. New attempt data makes an existing draft stale and blocks approval until refresh. Ambiguous findings require explicit uncertainty acknowledgment.
...
| Route | Purpose |
|---|---|
| `POST /api/attempts` | Ingest a delivery attempt using the ingestion key |
...
The CI gate runs 32 focused tests and 17 synthetic scenario classifications. It covers conflicting identities, account isolation, stale evidence, uncertain review, retries, a lost acknowledgment, concurrent duplicate ingestion, atomic batch import, persistence across restarts, API auth, malformed model output, and the HTTPS adapter. `examples/review_rubric.md` describes the manual answer-quality assessment required before a pilot. The fixture pass rate is **not** an estimate of production precision or model accuracy.
